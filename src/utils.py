...
class DropContradictoryAsthmaReports(BaseEstimator, TransformerMixin):
    """
    Drop rows where self-reports of asthma at two different time points by Mother and Father are contradictory.
    """

    def __init__(self, drop_if_nan=True):
    ...
    def transform(self, X, y=None):
        if self.drop_if_nan:
            # Drop rows where one of the two asthma histories is NaN
            X = X.dropna(subset=['Mother_Asthma', 'Asthma_Mother', 'Father_Asthma', 'Asthma_Father'])
        print(len(X))
        # Drop rows where Mother_Asthma and Asthma_Mother differ, only if both are not NaN
        X = X[(X['Mother_Asthma'].isna()) | (X['Asthma_Mother'].isna()) | (X['Mother_Asthma'] == X['Asthma_Mother'])]
        
        # Drop rows where Father_Asthma and Asthma_Father differ, only if both are not NaN
        X = X[(X['Father_Asthma'].isna()) | (X['Asthma_Father'].isna()) | (X['Father_Asthma'] == X['Asthma_Father'])]
        print(len(X))
        return X
...
class CytokinePercentileTransformer(BaseEstimator, TransformerMixin):
    """
    Create categorical features indicating whether cytokine levels fall into specific percentile ranges.
    
    This transformer identifies cytokine measurement columns and creates new binary features
    that indicate whether each cytokine level:
    1. Exceeds the upper percentile threshold (default: 75th percentile)
    2. Falls below the lower percentile threshold (default: 25th percentile)
    3. Falls between the lower and upper percentile thresholds
    
    These categorical features can be useful for identifying patterns in cytokine profiles
    that may be associated with asthma or other conditions.

    Parameters:
    ----------
    cytokine_cols : list or None, default=None
        List of column names containing cytokine measurements. If None, will try to identify
        columns with 'cytokine' or 'cyt_' in their name.
    
    upper_percentile : float, default=75.0
        The upper percentile threshold (0-100) to use for classifying elevated cytokine levels.
    
    lower_percentile : float, default=25.0
        The lower percentile threshold (0-100) to use for classifying reduced cytokine levels.
    
    elevated_suffix : str, default='_elevated'
        Suffix to append to the cytokine column names for elevated features (above upper percentile).
        
    reduced_suffix : str, default='_reduced'
        Suffix to append to the cytokine column names for reduced features (below lower percentile).
        
    normal_suffix : str, default='_normal'
        Suffix to append to the cytokine column names for normal features (between percentiles).
        
    drop_original : bool, default=True
        Whether to drop the original cytokine columns after creating the binary features.
    
    Returns:
    -------
    Transformed DataFrame with additional binary features indicating cytokine level categories.
    """

    def __init__(self, cytokine_cols=None, upper_percentile=75.0, lower_percentile=25.0, 
                elevated_suffix='_elevated', reduced_suffix='_reduced', 
                normal_suffix='_normal', drop_original=True):
        self.cytokine_cols = cytokine_cols
        self.upper_percentile = upper_percentile
        self.lower_percentile = lower_percentile
        self.elevated_suffix = elevated_suffix
        self.reduced_suffix = reduced_suffix
    ...
    def fit(self, X, y=None):
        """
        Calculate the specified percentile thresholds for each cytokine column.
        
        Parameters:
        ----------
        X : pandas DataFrame
            The input DataFrame containing cytokine measurements.
        ...
        self
        """
        # Identify cytokine columns if not specified
        if self.cytokine_cols is None:
            self.cytokine_cols = [col for col in X.columns if 'cytokine' in col.lower() or 'cyt_' in col.lower()]
            if not self.cytokine_cols:
                print("Warning: No cytokine columns identified. Please specify cytokine_cols parameter.")
        
        # Calculate the thresholds for each cytokine
        for col in self.cytokine_cols:
            if col in X.columns:
                self.upper_thresholds_[col] = X[col].quantile(self.upper_percentile / 100.0)
                self.lower_thresholds_[col] = X[col].quantile(self.lower_percentile / 100.0)
        
        print(f"Cytokine thresholds:")
        for col in self.upper_thresholds_:
            print(f"  {col}:")
            print(f"    {self.lower_percentile}th percentile: {self.lower_thresholds_[col]:.3f}")
            print(f"    {self.upper_percentile}th percentile: {self.upper_thresholds_[col]:.3f}")
            
        return self

    def transform(self, X, y=None):
        """
        Create binary features indicating whether cytokine levels fall into specific percentile ranges.
        ...
        -------
        pandas DataFrame
            Transformed DataFrame with additional binary features for categorized cytokines.
        """
        X_transformed = X.copy()
        
        # Create binary features for each cytokine
        for col in self.upper_thresholds_:
            if col in X.columns:
                # Elevated feature (above upper percentile)
                elevated_col = f"{col}{self.elevated_suffix}"
                X_transformed[elevated_col] = (X[col] > self.upper_thresholds_[col]).astype(int)
                
                # Reduced feature (below lower percentile)
                reduced_col = f"{col}{self.reduced_suffix}"
                X_transformed[reduced_col] = (X[col] < self.lower_thresholds_[col]).astype(int)
                
                # Normal feature (between lower and upper percentiles)
                normal_col = f"{col}{self.normal_suffix}"
                X_transformed[normal_col] = (
                    (X[col] >= self.lower_thresholds_[col]) & 
                    (X[col] <= self.upper_thresholds_[col])
                ).astype(int)
        
        # Drop original cytokine columns if requested
        if self.drop_original:
            cols_to_drop = [col for col in self.upper_thresholds_ if col in X_transformed.columns]
...
class DropHighNaNColumns(BaseEstimator, TransformerMixin):
    """
    Drop columns that have a number of NaN values surpassing a specified cutoff.

    Parameters:
    ----------
    cutoff: float, default 0.5
        The proportion of NaN values in a column to use as the cutoff for dropping the column.
    """
    ...
    def transform(self, X, y=None):
        # Calculate the proportion of NaN values per column
        nan_proportion = X.isna().mean()

        # Identify columns to drop
        columns_to_drop = nan_proportion[nan_proportion > self.cutoff].index

        # Drop the identified columns
        X_transformed = X.drop(columns=columns_to_drop)
...
class CatNaNImputer(BaseEstimator, TransformerMixin):
    ...
    Parameters:
    ----------
    NaN_signal_thresh: int, default 30
        The threshold for the number of NaN values to signal imputation.
    NaN_imputation_strategy: str, default "mode"
        The strategy to use for imputation. Options are "mode" or "constant".
    constant_value: any, default None
        The constant value to use for imputation if NaN_imputation_strategy is "constant".
    """

    def __init__(self, NaN_signal_thresh=30, NaN_imputation_strategy="mode", constant_value=None):
    ...
    def transform(self, X, y=None):
        ...
        for col in X_transformed.columns:
            # Count the number of distinct values (excluding NaN)
            distinct_values = X_transformed[col].dropna().nunique()

            # If there are only 2 distinct values, treat it as a categorical variable
            if distinct_values == 2:
                if self.NaN_imputation_strategy == "mode":
                    # Impute with the mode
                    mode_value = X_transformed[col].mode()[0]
                    X_transformed[col] = X_transformed[col].fillna(mode_value)
                elif self.NaN_imputation_strategy == "constant":
                    # Impute with the constant value
                    X_transformed[col] = X_transformed[col].fillna(self.constant_value)
...
class AssignProbableAndDefiniteAsthma(BaseEstimator, TransformerMixin):
    """
    Transform 'Asthma_Diagnosis_5yCLA' and 'Asthma_Diagnosis_3yCLA' into new columns:
    - 'definite_asthma_5y': 1 if 'Asthma_Diagnosis_5yCLA' is 1, otherwise 0
    - 'definite_asthma_3y': 1 if 'Asthma_Diagnosis_3yCLA' is 1, otherwise 0
    - 'emergent_asthma': 1 if 'Asthma_Diagnosis_3yCLA' is 0 and 'Asthma_Diagnosis_5yCLA' is 1, otherwise 0
    - 'transient_asthma': 1 if 'Asthma_Diagnosis_3yCLA' is 1 and 'Asthma_Diagnosis_5yCLA' is 0, otherwise 0
    - 'persistent_asthma': 1 if 'Asthma_Diagnosis_3yCLA' is 1 and 'Asthma_Diagnosis_5yCLA' is 1, otherwise 0
    Drop rows where 'Asthma_Diagnosis_5yCLA' or 'Asthma_Diagnosis_3yCLA' is 2 (indicating possible asthma).
    """

    def __init__(self, column_name_5y='Asthma_Diagnosis_5yCLA', column_name_8y='Asthma_Diagnosis_8yCLA', column_name_3y='Asthma_Diagnosis_3yCLA'):
        self.column_name_5y = column_name_5y
        self.column_name_3y = column_name_3y
        self.column_name_8y = column_name_8y
    ...
    def transform(self, X, y=None):
        ...
        # Drop rows where 'Asthma_Diagnosis_5yCLA' or 'Asthma_Diagnosis_3yCLA' is null or 2 or 'Asthma_Diagnosis_8yCLA' is null or 3
        X_transformed = X_transformed.dropna(subset=[self.column_name_5y, self.column_name_3y])
        X_transformed = X_transformed[(X_transformed[self.column_name_5y] != 2) & (X_transformed[self.column_name_3y] != 2)]
        X_transformed = X_transformed[(X_transformed[self.column_name_8y] != 3)]

        # Create 'definite_asthma_5y' column
        X_transformed['definite_asthma_5y'] = X_transformed[self.column_name_5y].apply(lambda x: 1 if x == 1 else 0)

        # Create 'definite_asthma_3y' column
        X_transformed['definite_asthma_3y'] = X_transformed[self.column_name_3y].apply(lambda x: 1 if x == 1 else 0)

         # Create 'definite_asthma_3y' column
        X_transformed['definite_asthma_8y'] = X_transformed[self.column_name_8y].apply(lambda x: 1 if x == 1 else 0)

        # Create 'emergent_asthma' column
        X_transformed['emergent_asthma_at_5y'] = X_transformed.apply(lambda row: 1 if row[self.column_name_3y] == 0 and row[self.column_name_5y] == 1 else 0, axis=1)

        
        # Create 'persistent_asthma_at_5y' column
        X_transformed['persistent_asthma_at_5y'] = X_transformed.apply(lambda row: 1 if row[self.column_name_3y] == 1 and row[self.column_name_5y] == 1 else 0, axis=1)

        X_transformed['emergent_asthma_at_8y'] = X_transformed.apply(lambda row: 1 if row[self.column_name_3y] == 0 and row[self.column_name_8y] == 1 else 0, axis=1)


        # Create 'persistent_asthma_at_8y' column
        X_transformed['persistent_asthma_at_8y'] = X_transformed.apply(lambda row: 1 if row[self.column_name_5y] == 1 and row[self.column_name_8y] == 1 else 0, axis=1)



        # Drop the 'Asthma_Diagnosis_5yCLA' and 'Asthma_Diagnosis_3yCLA' columns
        X_transformed = X_transformed.drop(columns=[self.column_name_5y, self.column_name_3y, self.column_name_8y])

        return X_transformed

class OneHotEncoder(BaseEstimator, TransformerMixin):
    """
    OneHotEncoder for all categorical columns, with drop_first for binary columns.

    Parameters:
    ----------
    drop_first: boolean, default True
        Whether to drop the first level of binary columns to avoid multicollinearity.
    handle_na: boolean, default True
        Whether to handle NaN values by converting them to a specified value.
    na_value: object, default 'Missing'
        The value to replace NaN values with.
    """

    def __init__(self, drop_first=True, handle_na=True, na_value='Missing'):
        super().__init__()
        self.drop_first = drop_first
        self.handle_na = handle_na
        self.na_value = na_value
        self.columns_to_encode = ['Sex','Study_Center','BF_Status_3m','BF_Status_6m','Child_Ethnicity','Mode_of_delivery','PSS_36week','CESD_36week','PSS_18week','CESD_18week','PSS_6m','CESD_6m','PSS_12m','CESD_12m','PSS_18m','CESD_18m','PSS_24m','CESD_24m']
        #self.columns_to_encode = ['Sex','Study_Center','BF_Status_3m','BF_Status_6m','Child_Ethnicity','Mode_of_delivery']
    ...
    def fit(self, X, y=None):
        ...
        if self.columns_to_encode is None:
            ...
            self.columns_to_encode = [col for col in self.columns_to_encode if col in X.columns]

        self.columns_to_encode = [
            (col, X[col].nunique() == 2 and self.drop_first) for col in self.columns_to_encode
        ]
    ...
    def transform(self, X, y=None):
        ...
        for col, drop_first in self.columns_to_encode:
            if self.handle_na:
                if X_transformed[col].dtype.name == 'category':
                    X_transformed[col] = X_transformed[col].cat.add_categories([self.na_value])
                X_transformed[col].fillna(self.na_value, inplace=True)

            if drop_first:
                encoded_cols = pd.get_dummies(X_transformed[col], prefix=col, drop_first=True)
            else:
                encoded_cols = pd.get_dummies(X_transformed[col], prefix=col, drop_first=False)
...
class CollinearRemover(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        print(self.repetition_drop)
        return X.drop(columns=list(set(self.repetition_drop)), inplace=False)
